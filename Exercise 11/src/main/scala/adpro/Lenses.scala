...
object Lenses {
  ...
  val l2 = Lens[String, (String,Int)] ((s: String) => (s,0)) (s1 => _ => (s1._1))

  // page 7 in Foster et al.

  val l3 = Lens[(String,Int), String] (_._1) (s => t => if (s == t._1) (s,t._2) else (s, t._2+1))
  ...
  def codiag[A]: Lens[A \/ A, A] = {
    val l = Lens[A, A] (identity) (v => _ => v)
    val r = Lens[A, A] (identity) (v => _ => v)
    Lens[A \/ A, A] (c => c match {
        case -\/(a) => l.get(a)
        case \/-(b) => r.get(b)
      }
    ) (a => c => c match {
