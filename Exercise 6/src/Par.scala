...
object Par {
  ...
  def sequence[A] (ps: List[Par[A]]): Par[List[A]] =
    ps.foldRight[Par[List[A]]] (unit(List.empty))(map2(_,_)(_::_))
  ...
  def parFilter[A] (as: List[A])
    ...
    fork {
      val fas = as.map (asyncF (a => if (f (a)) List(a) else empty[A]))
  ...
  def map3[A,B,C,D] (pa: Par[A], pb: Par[B], pc: Par[C])
                    (f: (A, B, C) => D): Par[D] =
    map2 (map2 (pa,pb)((a,b) => (c: C) => f (a,b,c)), pc)((g,c) => g(c))

  // shown in the book
  def equal[A] (e: ExecutorService)
               (p: Par[A], p2: Par[A]): Boolean =
    p (e).get == p2 (e).get
  ...
  def choice[A] (cond: Par[Boolean])
    ...
    def i2b: Par[Int] = (es: ExecutorService) => if (run(es)(cond).get) UnitFuture(0) else UnitFuture(1)
    choiceN (i2b)(List(t,f))
  }
  ...
  def choiceNviaChooser[A] (n: Par[Int])
                           (choices: List[Par[A]]): Par[A] =
    chooser(n)(choices)

  def choiceViaChooser[A] (cond: Par[Boolean])
                          (t: Par[A], f: Par[A]): Par[A] =
